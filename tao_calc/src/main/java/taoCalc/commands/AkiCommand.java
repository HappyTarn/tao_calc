...
public class AkiCommand extends Command {
	...
	protected void execute(CommandEvent event) {
		...
		for (String channelId : channels) {
			...
			String userId = channelManager.getEDate(channelId);
			String msg = channelManager.getFDate(channelId);
			if (date == null) {
				if (messageChannel == null) {
					eb.addField("チャンネル名取得中...", "【空き】<#" + channelId + ">", false);
				} else {
			...
			if (now.after(calendar.getTime())) {
				if(msg.equals(Const.超激レア出現) || msg.equals(Const.トール出現)) {
					eb.addField(messageChannel.getName(), "【" + msg + "中】" + userId, false);
				}else {
					eb.addField(messageChannel.getName(), "【空き】<#" + channelId + ">", false);
				}
			} else {
